...
# ~/.local/bin. Used only as a fallback when Homebrew isn't available.
# Deliberately does NOT use the curl|sh installer at get.chezmoi.io: in testing
# on Linux it failed intermittently in ways that were hard to diagnose (the
