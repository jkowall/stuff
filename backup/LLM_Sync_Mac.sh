...
resolve_machine_name() {
    ...
    if [[ -n "$explicit_name" ]]; then
        print -r -- "$explicit_name"
        return 0
    fi

    local raw_name
    raw_name=$(scutil --get LocalHostName 2>/dev/null)
    if [[ -z "$raw_name" ]]; then
        raw_name=$(hostname)
    fi

    # Avoid using Bonjour/domain suffixes like ".local" in backup folder names.
    print -r -- "${raw_name%%.*}"
}
...
backup_portable_app_configs() {
    ...
    local codex_app="$HOME/Library/Application Support/Codex"
    copy_file_if_exists "$codex_app" "browser-sidebar-local-servers.json" "$root/codex"

    local openai_codex_app="$HOME/Library/Application Support/OpenAI/Codex"
...
path_signature() {
    ...
    if [[ -f "$target_path" ]]; then
        shasum -a 256 "$target_path" | awk '{print "FILE:" $1}'
        return 0
    fi
    find "$target_path" -type f -print0 | sort -z | xargs -0 shasum -a 256 | sed "s|$target_path/||" | tr '\n' '|'
}
...
create_safety_backup() {
    ...
    local safe_label
    safe_label=$(print -r -- "$restore_label" | tr '\\/:*?"<>|' '_')
    ...
    if [[ -d "$pre_restore_root" ]]; then
        if [[ "$DRY_RUN" == "true" ]]; then
            ...
            ls -dt "$pre_restore_root"/backup_* 2>/dev/null | tail -n +3 | xargs rm -rf
        fi
    fi
}
...
if [[ "$ACTION" == "backup" ]]; then
    ...
    if [[ "$VERSIONED" == "true" ]]; then
        ...
        done < <(find "$BASE_BACKUP_DIR" -maxdepth 1 -type d -name "${HOSTNAME}_*" -mtime +30)

        if [[ ${#OLD_BACKUPS[@]} -gt 0 ]]; then
            if confirm "Prune old backups"; then
                if [[ "$DRY_RUN" == "true" ]]; then
                    for directory in "${OLD_BACKUPS[@]}"; do
                        echo "[dry-run] rm -rf \"$directory\""
                    done
                else
                    for directory in "${OLD_BACKUPS[@]}"; do
                        rm -rf "$directory"
    ...
    machine_display=()
    while IFS= read -r directory; do
        mod_date=$(stat -f "%Sm" -t "%Y-%m-%d %H:%M" "$directory")
