...
path_signature() {
    ...
    find "$path" -type f -print0 | sort -z | xargs -0 shasum -a 256 | sed "s|$path/||" | tr '\n' '|'
}
...
preview_restore_diffs() {
    local root=$1
    read -q "preview_choice?Preview text diffs before restore? (y/n): "
    echo
...
if [[ "$ACTION" == "backup" ]]; then
    ...
    read -q "push_choice?Push backup changes to Git? (y/n): "
    echo
